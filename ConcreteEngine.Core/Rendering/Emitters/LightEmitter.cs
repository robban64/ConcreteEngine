...
public sealed class LightEmitter : DrawCommandEmitter<DrawCommandLight>
{
    ...
    public LightEmitter()
    {
        ...
        for (int i = 0; i < 64; i++)
        {
            var l = new DrawCommandLight(new(r.Next(0, 2048), r.Next(0, 2048)), new(r.NextSingle(), r.NextSingle(), r.NextSingle()), r.Next(40, 200),
                r.NextSingle()*2);
            _lights.Add(l);
        }
    }
    
    protected override void EmitBatch(ReadOnlySpan<DrawCommandLight> entities, in DrawEmitterContext ctx, DrawCommandSubmitter submitter, int order)
    {
