...
internal static class WorldParamsComponent
{
    ...
    private static void DrawFogState()
    {
        ...
        ImGui.DragFloat("##FogDensity", ref fog.Density, 0.0001f, 0.0001f, 1.0f, "Den: %.5f");
        fieldStatus.NextField();

        ImGui.TextUnformatted("Base Height");
        ImGui.DragFloat("##FogBaseHeight", ref fog.BaseHeight, 0.1f, -1000f, 1000f, "Hgt: %.1f");
        fieldStatus.NextField();

        ImGui.TextUnformatted("Height Falloff");
        ImGui.DragFloat("##FogHeightFalloff", ref fog.HeightFalloff, 0.001f, 0.001f, 10000.0f, "Fall: %.3f");
        fieldStatus.NextField();
