...
internal sealed class InteractionController(ApiContext apiContext)
{
    ...
    private DragEntityState _dragState = default;
    ...
    public EntityId OnClick(Vector2 mousePosition, out BoundingBox bounds, out float distance)
    {
        ...
        return _selectedEntityId = raycaster.GetEntityByCameraRay(mousePosition, out bounds, out distance);
    }

    public void OnDragEntity(Vector2 mousePosition)
    {
    ...
    private void OnDragStart(Vector2 mousePosition)
    {
        ...
        ref readonly var transform = ref GetTransform(_selectedEntityId);

        var pointOnTerrain = world.Raycast.GetPointOnTerrain(mousePosition, out var bounds);
