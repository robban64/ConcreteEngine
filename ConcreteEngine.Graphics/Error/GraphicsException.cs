...
public sealed class GraphicsException : InvalidOperationException
{
    public GraphicsException(string message) : base(message)
    {
    }

    // --- Factories (now concise) ---

    public static GraphicsException ResourceNotBound<T>(string? name = null) =>
        new($"{Label<T>(name)} is not bound to the pipeline.");

    public static GraphicsException ResourceIsDisposed<T>(string? name = null) =>
        new($"{Label<T>(name)} is disposed.");
