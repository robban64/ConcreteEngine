...
public readonly ref struct NativeAllocator(NativeView<byte> view, ref int cursor, int alignment = 0)
{
    public readonly NativeView<byte> View = view;
    public readonly int Alignment = alignment;
    public readonly ref int Cursor = ref cursor;

    public int Length => View.Length;
    public int Remaining => View.Length - Cursor;
    ...
    public NativeView<byte> AllocSlice(int length)
    {
        ...
        var start = Cursor;
        Cursor += length;
        return View.Slice(start, length);
    }
    ...
    public NativeView<byte> AllocStringSlice(ReadOnlySpan<char> str, bool nullTerminated = true)
    {
        ...
        int written = Encoding.UTF8.GetBytes(str, View.AsSpan());
