...
internal sealed class ModelInspectorUi(PanelContext panelContext, AssetController assetController)
{
    public void Draw(InspectModel editModel, in FrameContext ctx)
    {
        ...
        AppDraw.DrawTextProperty("Vertices:"u8, ref ctx.Sw.Write(model.Info.VertexCount));
        AppDraw.DrawTextProperty("Triangles:"u8, ref ctx.Sw.Write(model.Info.FaceCount));
        AppDraw.DrawTextProperty("Meshes:"u8, ref ctx.Sw.Write((int)model.Info.MeshCount));
        ...
        foreach (var mesh in editModel.Asset.Meshes)
        {
            ...
            var spec = mesh.Info;
            AppDraw.DrawTextProperty("Vertices:"u8, ref ctx.Sw.Write(spec.MeshIndex));
            AppDraw.DrawTextProperty("MatIndex:"u8, ref ctx.Sw.Write(spec.MaterialIndex));
            AppDraw.DrawTextProperty("Vertices:"u8, ref ctx.Sw.Write(spec.VertexCount));
            AppDraw.DrawTextProperty("Triangles:"u8, ref ctx.Sw.Write(spec.TrisCount));
    ...
    private static void DrawAnimated(ModelAnimation animation, FrameContext ctx)
    {
        ...
        AppDraw.DrawTextProperty("Bone Count:"u8, ref ctx.Sw.Write(animation.BoneCount));
