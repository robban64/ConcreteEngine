...
internal static partial class PassPipeline
{
    ...
    public static void RegisterPassPipeline()
    {
        ...
        registry.RegisterPass<ShadowTarget>(FboVariant.V0, PassOp.Draw, MakeShadow())
            .OnPassBegin(static ctx =>
            {
                ...
                gfx.BindTextureAndSampler(RenderContext.DepthTexture, SamplerProfile.ShadowCompare,
                    SamplerSlot.ShadowMap0);
        ...
        registry.RegisterPass<SceneTarget>(FboVariant.V0, PassOp.Draw, MakeSceneMsaa())
            .OnPassBegin(static ctx =>
            {
                ...
                ctx.Gfx.BindTextureAndSampler(RenderContext.DepthTexture, SamplerProfile.ShadowCompare,
                    SamplerSlot.ShadowMap0);
                return PassAction.DrawPassResult();
            })
            .OnPassEnd(static ctx =>
    ...
    private static GfxPassState MakeShadow() =>
        GfxPassState.MakeDepthClear(DepthTest | DepthWrite | Cull | Srgb | PolygonOffset | Ac2);
